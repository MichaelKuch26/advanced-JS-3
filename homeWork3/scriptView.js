...
for (let index = 0; index < localStorage.length; index++) {
  ...
  wrapProduct.insertAdjacentHTML("beforeend", `<h2>${product} <button onclick="showRewiew(this)">показать отзывы</button> </h2>`
  );
  ...
  rewiewList.forEach(element => {
    ...
    rewiewItem.insertAdjacentElement("beforeend", createdButtonDelete(rewiewText, rewiewList, product));
